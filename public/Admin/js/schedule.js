layui.use(['form', 'laydate', 'table', 'laytpl', 'jquery'], function () {
    ...
    var tableIns = table.render({
        elem: '#infoList',
        method: 'POST',
        url: '/admin.php/Activity/schedule/', //数据接口
        cellMinWidth: 50,
        id: "infoList",
        cols: [[
            { type: "checkbox", width: 50 },
            { field: 'id', title: 'ID', width: 80, align: "center", sort: true },
            { field: 'name', title: '日程名称', align: "center", edit: 'text' },
            { field: 'seo', title: '日程备注', align: "center", edit: 'text' },
            {
                field: 'time', title: '时间', align: 'center', width: 160, templet: function (d) {
                    return getLocalTime(d.statime);
                }
            },
            ...
            $(".layui-input-date").each(function (i) {
                ...
                laydate.render({
                    ...
                    range: true, //或 range: '~' 来自定义分割字符
                        ...
                        $.ajax({
                            type: "post",
                            url: "/admin.php/Activity/scheduleAdd",
                            dataType: 'json',
                            data: {
                                id: id,
                                time: value
    ...
    table.on('edit(infoList)', function (obj) { //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        $.ajax({
            type: "post",
            url: "/admin.php/Activity/scheduleEdit",
            dataType: 'json',
            data: {
                id: obj.data.id,
                field: obj.field,
    ...
    table.on('tool(infoList)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            var index = layer.confirm('真的删除行么', function (index) {
                ...
                $.ajax({
                    type: "post",
                    url: "/admin.php/Activity/scheduleDel",
                    dataType: 'json',
                    data: obj.data,
                    success: function (res) {
                        layer.close(index);
